...
public class ProductDBImpl implements ProductDB {
    ...
    public Product findProductByBarcode(String barcode) {
        ...
        try {
            connection = ConnectionDB.INSTANCE.getConnection();
            PreparedStatement ps = connection.prepareStatement("select p.id, p.name, p.category_id,p.price,p.unit_id, p.barcode,p.amount,p.discount, c.name, u.unit_name \n" +
                    "from products p join categories c \n" +
                    "on p.category_id = c.id join units u on p.unit_id = u.id \n" +
                    "where barcode = ?");
